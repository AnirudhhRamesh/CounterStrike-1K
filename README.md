...
Uses the dependency-free RGB-histogram encoder against the 2 GB sample repo. `--eval-split train` is needed because the sample repo only ships `split=train` rows; drop it for the full release (the default is `test` for paper reproduction).
